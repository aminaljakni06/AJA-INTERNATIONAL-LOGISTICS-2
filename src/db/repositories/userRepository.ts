...
function toUserDoc(user: User): UserDoc & { companyName?: string | null } {
  return {
    ...
    forcePasswordChange: (user as User & { forcePasswordChange?: boolean }).forcePasswordChange,
    metadata: (user as User & { metadata?: Record<string, unknown> }).metadata,
...
export async function updateUser(id: string, updates: Partial<UserDoc>): Promise<UserDoc> {
  ...
  if (useLocalUserStore()) {
    ...
    data.users[index] = {
      ...
      ...(payload.roles ? { roles: payload.roles as User['roles'] } : {}),
      ...(payload.permissions ? { permissions: payload.permissions as string[] } : {}),
      ...(payload.customPermissions ? { customPermissions: payload.customPermissions as string[] } : {}),
      ...(payload.companyId !== undefined ? { companyId: payload.companyId as string | null } : {}),
      ...(payload.branchId !== undefined ? { branchId: payload.branchId as string | null } : {}),
      ...(payload.departmentId !== undefined ? { departmentId: payload.departmentId as string | null } : {}),
      ...(payload.securityLevel !== undefined ? { securityLevel: payload.securityLevel as number } : {}),
      ...(payload.forcePasswordChange !== undefined ? { forcePasswordChange: payload.forcePasswordChange as boolean } : {}),
